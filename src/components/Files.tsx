import { onMount, onCleanup, createSignal, Show, For, type Component } from 'solid-js';
...
const Files: Component<{ toggle: (param: string) => void }> = (props) => {
  ...
  let [filesPos, setFilesPos] = createSignal({ top: window.innerHeight, left: window.innerWidth / 3 });
  let [windowMaximized, setWindowMaximized] = createSignal(false);

  onMount(() => {
    if (files) {
      setFilesPos({ top: Math.random() * (window.innerHeight - files.offsetHeight), left: Math.random() * (window.innerWidth - files.offsetWidth) });
  ...
  return (
    <div ref={files!} class={`w-[45vw] h-[40vh] flex justify-start absolute shadow `}>
      <div class="h-full w-1/4 bg-neutral-100/75 backdrop-blur-md pt-2 rounded-tl-lg rounded-bl-lg">
        ...
        <For each={shortcuts}>
          {(s: any) => (
            <div class="text-sm w-full text-neutral-700 p-1 hover:bg-neutral-100/25 hover:cursor-default">
              <Icon icon={s.icon} /><span>{s.title} </span>
            </div>
          )}
        </For>
      </div>
