...
const Terminal: Component<{ toggle: (param: string) => void }> = (props) => {

  let terminal: HTMLDivElement;
  let [cmd, setCmd] = createSignal('')
  let [terminalPos, setTerminalPos] = createSignal<{ top: number; left: number }>({ top: window.innerHeight, left: window.innerWidth / 2 })
  let [windowMaximized, setWindowMaximized] = createSignal(false);

  let [history, setHistory] = createSignal<string[]>([])
  onMount(() => {
    if (terminal) {
      setTerminalPos({ top: Math.random() * (window.innerHeight - terminal.offsetHeight - 50), left: Math.random() * (window.innerWidth - terminal.offsetWidth) });
  ...
  return (
    ...
    >
      ...
      <div class="grid pt-2 px-2">
        ...
        <For each={history()}>
          {(h) => (
            <p class="text-white" innerHTML={h}></p>
          )}
        </For>
